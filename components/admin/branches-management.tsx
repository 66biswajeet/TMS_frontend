...
import { fetchBranches, createBranch, updateBranch, deleteBranch, assignUsersToBranch } from "@/redux/modules/branches/actions"
...
import { Search, Plus, Edit, Trash2, Building, MapPin, Users, TrendingUp, X } from "lucide-react"
...
export function BranchesManagement() {
  ...
  const { items: branches, loading, error } = useSelector((state: RootState) => state.branches)
  ...
  const [selectedStaffForRemoval, setSelectedStaffForRemoval] = useState<string[]>([])
  ...
  const [selectedAreaManagerId, setSelectedAreaManagerId] = useState<string>("")
  const [selectedAuditorId, setSelectedAuditorId] = useState<string>("")
  const [selectedBranchIds, setSelectedBranchIds] = useState<string[]>([])
  const [selectedAreaManagerIds, setSelectedAreaManagerIds] = useState<string[]>([])
  ...
  const fetchBranchManagers = async () => {
    try {
      const response = await api.get('/branches/users-by-role?role=branch_manager')
      setAllUsers(response.data.items || [])
    } catch (error) {
  ...
  const handleAssignBranchManager = async (branch: Branch, userId: string) => {
    try {
      ...
      const isAlreadyAssigned = selectedManager && selectedManager.AssignedBranches && selectedManager.AssignedBranches.trim() !== ''
      ...
      await api.post(`/branches/${branch.BranchId}/assign-branch-manager`, { userId })
  ...
  const handleRemoveStaffFromBranch = async (userId: string, branch: Branch) => {
    try {
  ...
  const handleStaffSelectionForRemoval = (userId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedStaffForRemoval(selectedStaffForRemoval.filter(id => id !== userId))
    }
  }
  ...
  const handleBulkRemoveUsers = async (branch: Branch) => {
    ...
    try {
      ...
      if (successCount > 0 && failedCount === 0) {
        showSuccess(`${successCount} staff member(s) removed from ${branch.BranchName} successfully`)
      } else if (successCount > 0 && failedCount > 0) {
        showWarning(`${successCount} staff member(s) removed successfully, but ${failedCount} failed to remove.`)
      } else {
      ...
      showError('An error occurred during the removal process. Please check the console for details.')
    }
  }

  // Handle assign area manager
  const handleAssignAreaManager = async (userId: string, branchIds: string[]) => {
    try {
  ...
  const handleAssignAuditorToAreaManagers = async (auditorId: string, areaManagerIds: string[]) => {
    try {
      await api.post('/branches/assign-auditor-to-area-managers', { auditorId, areaManagerIds })
  ...
  const handleAreaManagerSelectionChange = (managerId: string) => {
    ...
    const selectedManager = areaManagers.find(manager => manager.UserId === managerId)
    if (selectedManager && selectedManager.AssignedBranches) {
      ...
      const assignedBranchIds = branches
        .filter((branch: any) => assignedBranchNames.includes(branch.BranchName))
  ...
  const handleAuditorSelectionChange = (auditorId: string) => {
    ...
    const selectedAuditor = auditors.find(auditor => auditor.UserId === auditorId)
    if (selectedAuditor && selectedAuditor.SupervisedAreaManagerIds) {
      // Parse the supervised area manager IDs
      const supervisedIds = selectedAuditor.SupervisedAreaManagerIds.split(',').filter(id => id.trim())
  ...
  const handleAreaManagerSelectionForAuditor = (managerId: string, checked: boolean) => {
    if (checked) {
      setSelectedAreaManagerIds([...selectedAreaManagerIds, managerId])
    } else {
      setSelectedAreaManagerIds(selectedAreaManagerIds.filter(id => id !== managerId))
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Branches Management</h1>
          <p className="text-muted-foreground">Manage pharmacy branch locations and details</p>
        </div>
          ...
          <Card key={branch.BranchId} className="hover:shadow-md transition-shadow">
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <Badge variant={getStatusColor(branch.IsActive)} className="text-xs">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2 text-sm">
                <p className="text-muted-foreground">Location Code: {branch.LocationCode || "N/A"}</p>
              </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingBranch ? "Update branch information and details." : "Add a new pharmacy branch location."}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isManagerDialogOpen} onOpenChange={setIsManagerDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {selectedBranch ? `Assign Manager to ${selectedBranch.BranchName}` : "Assign Branch Manager"}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4">
            <div className="max-h-96 overflow-y-auto">
              {allUsers.length > 0 ? (
                <div className="space-y-2">
                  {allUsers.map((user) => (
                    <div key={user.UserId} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50">
                      <input
                        type="radio"
                        name="branchManager"
                        value={user.UserId}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {user.AssignedBranches && (
                            <Badge variant="outline" className="text-xs">
                              {user.BranchCount} branch{user.BranchCount !== 1 ? 'es' : ''}
                            </Badge>
                          )}
                        </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedBranch) {
                    ...
                    handleAssignBranchManager(selectedBranch, selectedManagerId || "")
                    setIsManagerDialogOpen(false)
                    setSelectedManagerId("")
                  }
                }}
      ...
      <Dialog open={isStaffDialogOpen} onOpenChange={setIsStaffDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {selectedBranch ? `Assign Staff to ${selectedBranch.BranchName}` : "Assign Staff"}
            </DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isUserDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {selectedBranch ? `Users in ${selectedBranch.BranchName}` : "Branch Users"}
            </DialogTitle>
            <DialogDescription>
              {selectedBranch ? `Manage users assigned to ${selectedBranch.BranchName}` : "Manage branch users"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {/* Header with buttons */}
                  ...
                  <>
                    ...
                    {selectedStaffForRemoval.length > 0 && (
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => selectedBranch && handleBulkRemoveUsers(selectedBranch)}
                      >
                        <X className="h-4 w-4 mr-2" />
                  ...
                  {branchUsers.map((user) => (
                    <div key={user.UserId} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50">
                      ...
                      {isRemovalMode && (
                        <input
                          type="checkbox"
                          checked={selectedStaffForRemoval.includes(user.UserId)}
                          onChange={(e) => handleStaffSelectionForRemoval(user.UserId, e.target.checked)}
                          className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                        />
                      )}
                      ...
                      {user.RoleRank === 5 && isRemovalMode && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (confirmRemove) {
                              handleRemoveStaffFromBranch(user.UserId, selectedBranch!)
                            }
                          }}
      ...
      <Dialog open={isAreaManagerDialogOpen} onOpenChange={setIsAreaManagerDialogOpen}>
                ...
                {areaManagers.length > 0 ? (
                  <div className="space-y-2">
                    {areaManagers.map((manager) => (
                      <div key={manager.UserId} className="flex items-center space-x-3 p-2 border rounded hover:bg-muted/50">
                        <input
                          type="radio"
                          name="areaManager"
                          value={manager.UserId}
                          checked={selectedAreaManagerId === manager.UserId}
                          onChange={(e) => handleAreaManagerSelectionChange(e.target.value)}
                          className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                        />
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {manager.Email} | Currently managing: {manager.BranchCount || 0} branches
              ...
              <div className="max-h-64 overflow-y-auto border rounded-lg p-2">
                <div className="grid grid-cols-2 gap-2">
                  {branches.map((branch: any) => (
                    <div key={branch.BranchId} className="flex items-center space-x-2 p-2 border rounded hover:bg-muted/50">
                      <input
                        type="checkbox"
                        checked={selectedBranchIds.includes(branch.BranchId)}
                        onChange={(e) => handleBranchSelectionChange(branch.BranchId, e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                      />
                      <div className="flex-1">
                        <div className="font-medium text-sm">{branch.BranchName}</div>
                        <div className="text-xs text-muted-foreground">{branch.GroupName}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedAreaManagerId) {
                    handleAssignAreaManager(selectedAreaManagerId, selectedBranchIds)
                  }
                }}
                disabled={!selectedAreaManagerId}
              >
                {selectedBranchIds.length > 0 ? "Assign Area Manager" : "Unassign Area Manager"}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={isAuditorDialogOpen} onOpenChange={setIsAuditorDialogOpen}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Select an auditor and the area managers they will supervise. Auditors will have access to all branches, branch managers, and staff under their assigned area managers.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6">
            {/* Auditor Selection */}
                ...
                {auditors.length > 0 ? (
                  <div className="space-y-2">
                    {auditors.map((auditor) => (
                      <div key={auditor.UserId} className="flex items-center space-x-3 p-2 border rounded hover:bg-muted/50">
                        <input
                          type="radio"
                          name="auditor"
                          value={auditor.UserId}
                          checked={selectedAuditorId === auditor.UserId}
                          onChange={(e) => handleAuditorSelectionChange(e.target.value)}
                          className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                        />
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm text-muted-foreground">
                            ...
                            {auditor.SupervisedAreaManagers && (
                              <div className="text-xs text-blue-600 mt-1">
                                Currently supervising: {auditor.SupervisedAreaManagers}
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <div className="space-y-2">
                  {areaManagers.map((manager) => (
                    <div key={manager.UserId} className="flex items-center space-x-3 p-2 border rounded hover:bg-muted/50">
                      <input
                        type="checkbox"
                        checked={selectedAreaManagerIds.includes(manager.UserId)}
                        onChange={(e) => handleAreaManagerSelectionForAuditor(manager.UserId, e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {manager.Email} | Managing: {manager.BranchCount || 0} branches
                          {manager.AssignedBranches && (
                            <div className="text-xs text-blue-600 mt-1">
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedAuditorId) {
                    handleAssignAuditorToAreaManagers(selectedAuditorId, selectedAreaManagerIds)
                  }
                }}
                disabled={!selectedAuditorId}
              >
                {selectedAreaManagerIds.length > 0 ? "Assign Auditor to Area Managers" : "Unassign Auditor from All Area Managers"}
              </Button>
            </div>
          </div>
        </DialogContent>
