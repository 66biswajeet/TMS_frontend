...
export function TaskHistory() {
  ...
  const [filter, setFilter] = useState<'all' | 'completed' | 'expired' | 'overdue'>('all')
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const matchesFilter = filter === 'all' || task.HistoryType.toLowerCase() === filter
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">View completed, expired, and overdue tasks</p>
        </div>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              >
                Completed ({tasks.filter(t => t.HistoryType === 'Completed').length})
              </Button>
              <Button
              ...
              >
                Expired ({tasks.filter(t => t.HistoryType === 'Expired').length})
              </Button>
              <Button
              ...
              >
                Overdue ({tasks.filter(t => t.HistoryType === 'Overdue').length})
              </Button>
            </div>
          </div>
        </CardContent>
        ...
        {filteredTasks.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-lg font-medium mb-2">No historical tasks found</h3>
              <p className="text-muted-foreground">
                {searchTerm ? 'Try adjusting your search terms' : 'Complete some tasks to see them here'}
              </p>
            </CardContent>
          </Card>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {getHistoryTypeBadge(task.HistoryType)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-muted-foreground">{task.BranchName || 'N/A'}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground">
                          {task.ChecklistCompleted}/{task.ChecklistTotal} items ({task.ChecklistPercentage}%)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-4 pt-4 border-t text-xs text-muted-foreground">
                      ...
                      <div>
                        <span className="font-medium">Updated:</span> {format(new Date(task.UpdatedAt), 'MMM dd, yyyy')}
                      </div>
                      <div>
                        <span className="font-medium">Relation:</span> {task.TaskRelation}
                      </div>
                    </div>
                  </div>
                </div>
