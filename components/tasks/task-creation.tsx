...
import { ArrowLeft, Plus, Trash2, GripVertical, Save, FileText, Search } from "lucide-react"
...
export function TaskCreation() {
  ...
  useEffect(() => {
    if (currentUser?.role === 'branch_manager' && branches.length > 0 && !formData.branchId) {
    ...
    } else if (currentUser?.role === 'staff' && branches.length === 1 && !formData.branchId) {
      setFormData(prev => ({ ...prev, branchId: branches[0].BranchId }))
    }
  ...
  const filteredUsers = (() => {
    ...
    if (currentUser?.role === 'branch_manager') {
      ...
      availableUsers = branchUsers.filter((user: any) =>
    ...
    // For area managers: show staff from branches they manage
      ...
      if (formData.branchId) {
        ...
        availableUsers = users.filter((user: any) =>
  ...
  useEffect(() => {
    fetchUsersInBranch(formData.branchId)
  }, [formData.branchId])
    ...
    if (template) {
      ...
      try {
        ...
        if (templateDetails.items && templateDetails.items.length > 0) {
          const items = templateDetails.items.map((item: any, index: number) => ({
            id: index + 1,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.scope === 'daily' && formData.isRepeating) {
        ...
        } else if (deadline && deadline.includes('T') && !deadline.includes(':')) {
      ...
      const taskData = {
        ...
        repeatEndDate: formData.isRepeating && formData.repeatEndDate ? formData.repeatEndDate : null,
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Set up a new task with checklist and assignments</p>
        </div>
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => setShowSaveTemplate(true)} className="bg-transparent">
        ...
        <Card className="max-w-2xl">
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-4 max-w-lg">
              ...
              <Button type="button" onClick={() => loadTemplate(selectedTemplate)} disabled={!selectedTemplate || !selectedTemplate.trim()}>
                Load Template
              </Button>
            </div>
          </CardContent>
      ...
      {showSaveTemplate && (
        <Card className="border-emerald-200 bg-emerald-50">
          <CardHeader>
            ...
            <CardDescription>Save this task configuration for future reuse</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 mb-20">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Enter the main details for your task</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
             ...
             {formData.scope === 'daily' && formData.isRepeating ? (
              ...
              {currentUser?.role === 'branch_manager' ? (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 p-3 border rounded-md bg-muted/50">
                    <span className="font-medium">
                      {formData.branchId ? branches.find(b => b.BranchId === formData.branchId)?.BranchName || 'Loading...' : 'Your Assigned Branch'}
                    </span>
                ...
                <div className="space-y-2">
                  <Label htmlFor="branchId">Branch</Label>
                  <div className="flex items-center gap-2 p-3 border rounded-md bg-muted/50">
                    <span className="font-medium">{branches[0].BranchName}</span>
                    <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">
                      Assigned Branch
                    </span>
                  </div>
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={
                        currentUser?.role === 'area_manager' ? "Select branch to assign task" :
                        currentUser?.role === 'auditor' ? "Select branch to audit" :
                        "Select branch"
                      } />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
                <div className="space-y-1">
                  <Label htmlFor="isRepeating" className="text-sm font-medium">Enable Repeating Task</Label>
                  <p className="text-xs text-muted-foreground">
              ...
              {formData.isRepeating && (
                <div className="space-y-6 pt-2">
                  <div className="grid grid-cols-1 gap-3">
                    <div className="space-y-2">
                      <Label htmlFor="repeatFrequency" className="text-sm font-medium">Repeat Frequency</Label>
                      <Select
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-2">
                        <Label htmlFor="repeatCount" className="text-sm font-medium">No. of Repetitions</Label>
                        <Input
                          id="repeatCount"
                          type="number"
                          min="1"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="repeatEndDate" className="text-sm font-medium">End Date (Optional)</Label>
                        <Input
                          id="repeatEndDate"
                          type="date"
                      ...
                      <div className="text-sm space-y-2">
                        ...
                        <ul className="text-blue-800 space-y-1 text-xs">
                          {formData.scope === 'daily' ? (
                            <>
                              <li>• Tasks become available immediately when created</li>
                              <li>• Must be completed by the specified time each day</li>
                              <li>• New task automatically created for the next day when completed</li>
                              <li>• Each task has the same assignees and checklist</li>
                              <li>• No "remaining days" - fresh daily completion cycle</li>
                            </>
                          ) : (
                            <>
                              <li>• New task instances created automatically based on frequency</li>
                              <li>• Each task has the same assignees and checklist</li>
                              <li>• Deadlines adjusted based on repeat frequency</li>
                              <li>• Tasks stop when end date reached or repeat count met</li>
                            </>
                          )}
                        </ul>
                      </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Create a checklist of items that need to be completed</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <Button type="button" variant="outline" onClick={addChecklistItem} className="bg-transparent px-4 py-2 w-full">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {currentUser?.role === 'branch_manager' ? (
                <p>Select staff members from your branch to complete this task</p>
              ) : currentUser?.role === 'area_manager' ? (
                <p>Select users from branches you manage</p>
              ) : currentUser?.role === 'auditor' ? (
                <p>Select users to audit from branches under your supervision</p>
              ) : (
          ...
          <CardContent className="space-y-4">
            ...
            <div className="max-h-80 overflow-y-auto space-y-2">
              {loadingBranchUsers ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  {currentUser?.role === 'branch_manager' && (
                    <div className="mt-2">
                      <p className="text-sm">You can only assign tasks to staff members in your branch</p>
                      <p className="text-xs mt-1">No staff members are currently assigned to your branch</p>
                    </div>
                  )}
                  {currentUser?.role === 'area_manager' && formData.branchId && (
                    <p className="text-sm mt-2">No eligible users found in this branch</p>
                  )}
                  {currentUser?.role !== 'branch_manager' && !formData.branchId && (
                    <p className="text-sm mt-2">Please select a branch first</p>
                  )}
                </div>
              ) : (
                filteredUsers.map((user: any, index: number) => {
                  ...
                  return (
                    <div key={uniqueKey} className={`flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50 ${!canAssign ? 'opacity-50' : ''}`}>
                      <Checkbox
                        id={userId}
                        checked={canAssign && formData.assigneeIds.includes(userId)}
                        onCheckedChange={(checked) => canAssign && handleAssigneeChange(userId, checked as boolean)}
                        disabled={!canAssign}
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <Label htmlFor={userId} className={`font-medium ${canAssign ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
                        ...
                        <div className="text-sm text-muted-foreground">
                          ...
                          {!canAssign && (
                            <span className="text-red-500 ml-2">Cannot assign to yourself</span>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {formData.assigneeIds.length === 0 && (
              <p className="text-sm text-red-600">Please select at least one assignee</p>
            )}
            
            {formData.assigneeIds.length > 0 && (
              <div className="text-sm text-muted-foreground">
                {formData.assigneeIds.length} user{formData.assigneeIds.length > 1 ? "s" : ""} selected
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="flex gap-3 pb-16 mb-16 justify-center">
          <Link href="/tasks">
            <Button type="button" variant="outline" className="bg-transparent px-6">
              Cancel
            </Button>
          </Link>
