...
import { ArrowLeft, Clock, Users, Building, CheckCircle, AlertTriangle, MessageSquare, ArrowUp } from "lucide-react"
...
export function TaskDetails({ taskId }: TaskDetailsProps) {
  ...
  const updateChecklistItem = async (itemId: string, completed: boolean, notes?: string) => {
    try {
      // Update local state immediately for responsive UI
      setChecklistItems((items) =>
        items.map((item) => (item.TaskChecklistItemId === itemId ? { ...item, Completed: completed, Notes: notes || item.Notes } : item)),
      )
      ...
      setChecklistItems((items) =>
        items.map((item) => (item.TaskChecklistItemId === itemId ? { ...item, Completed: !completed } : item)),
      )
      
      // Show more specific error message
      const errorMessage = error.response?.data?.error || "Failed to update checklist item. Please try again."
  ...
  const forwardTask = async () => {
    try {
      ...
      showWarning("Forward functionality requires selecting target user. This will be implemented based on business rules.")
  ...
  const canApproveTask = () => {
    ...
    const isAssigned = assignees.some(assignee => assignee.email === currentUser.email)
    if (isAssigned) return false
    
    // Rule 2: Only supervisors (branch managers, area managers, auditors, management) can approve
    const approverRoles = ['branch_manager', 'area_manager', 'auditor', 'management']
  ...
  const formatDeadline = (deadline: string) => {
    ...
    if (task?.Status === 'completed' || task?.Status === 'submitted' || task?.Status === 'approved') {
    ...
    if (diffHours < 0) {
      ...
      return { text: `Overdue by ${Math.abs(diffHours)}h${task?.AutoForward ? ' - Auto-forwarded' : ''}`, color: "text-red-600" }
    } else if (diffHours < 24) {
      return { text: `${diffHours}h remaining`, color: "text-amber-600" }
    } else {
      return { text: `${Math.ceil(diffHours / 24)}d remaining`, color: "text-green-600" }
    }
  }
  ...
  const isLocked = task.Status === "submitted" || task.Status === "approved" || isExpired
  
  // Check if current user is assigned to this task (only assignees can edit checklist)
  const canEditChecklist = assignees.some(assignee => assignee.email === currentUser?.email) && !isLocked
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <Badge variant={task.Priority === "high" ? "destructive" : "secondary"} className="text-sm">
            ...
            <div className="flex gap-2">
              {(task.Status === "in_progress" || task.Status === "Pending") && (
                <>
                  <Button variant="outline" onClick={forwardTask} disabled={submitting}>
              ...
              {task.Status === "submitted" && canApproveTask() && (
                <>
                  <Button variant="outline" onClick={rejectTask} disabled={submitting || !reviewReason.trim()}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${deadlineInfo.color}`}>{deadlineInfo.text}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{task.BranchName}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {completedItems}/{checklistItems.length} ({Math.round(progressPercentage)}%)
                </p>
              </div>
            </div>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                ...
                {task.Status === "expired" && "This task has expired and is read-only."}
                {!canEditChecklist && !isLocked && " Only assigned users can edit the checklist."}
              </CardDescription>
            </CardHeader>
              ...
              {checklistItems.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">
                    This task has no checklist items. Tasks created without checklist items will show 0/0 progress.
                  </p>
        ...
        <TabsContent value="workflow">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track the task progress through the organizational hierarchy</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {workflowSteps.map((step, index) => (
                  <div key={step.level} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      ...
                      />
                      {index < workflowSteps.length - 1 && <div className="w-px h-8 bg-border mt-2" />}
                    </div>
                    <div className="flex-1 pb-4">
                      ...
                      <p className="text-xs text-muted-foreground">{step.user || "Unassigned"}</p>
                      {step.completedAt && (
                      ...
                      {step.comments && (
                        <p className="text-sm text-muted-foreground mt-1 bg-muted p-2 rounded">{step.comments}</p>
                      )}
                    </div>
                  </div>
                ))}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {task.Status === "submitted" && canApproveTask() ? (
                ...
                <div className="text-center py-8">
                  <p className="text-muted-foreground">
                    You are not authorized to approve this task. Only supervisors who are not assigned to the task can approve it.
                  </p>
                </div>
              ) : (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">
                    Task must be submitted by assignees before it can be reviewed.
                  </p>
                </div>
              )}
            </CardContent>
        ...
        <TabsContent value="activity">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Timeline of all actions performed on this task</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
