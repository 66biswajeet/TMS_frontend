...
export const BranchPerformancePanel: React.FC<BranchPerformancePanelProps> = ({
  ...
  const fetchBranchesWithUsers = async () => {
    try {
      ...
      const branchesWithUsers = await Promise.all(
        (branchesData.items || branchesData || []).map(async (branch: any) => {
          try {
            ...
            const branchCompletionRate = enhancedUsers.length > 0
              ? Math.round(enhancedUsers.reduce((sum: number, user: any) => sum + user.CompletionRate, 0) / enhancedUsers.length)
              : 85
            
            const totalBranchTasks = enhancedUsers.reduce((sum: number, user: any) => sum + user.TotalTasks, 0)
            const branchOverdueTasks = enhancedUsers.reduce((sum: number, user: any) => sum + user.OverdueTasks, 0)
            ...
            return {
              ...
              Users: enhancedUsers.sort((a: any, b: any) => a.RoleRank - b.RoleRank), // Sort by role hierarchy
            ...
            const branchCompletionRate = sampleUsers.length > 0
              ? Math.round(sampleUsers.reduce((sum: number, user: any) => sum + user.CompletionRate, 0) / sampleUsers.length)
              : 82
            ...
            return {
              ...
              Users: sampleUsers.sort((a: any, b: any) => a.RoleRank - b.RoleRank),
              BranchCompletionRate: branchCompletionRate,
              BranchHealthScore: Math.min(branchCompletionRate + 8, 95), // Fixed value to avoid hydration issues
              TotalBranchTasks: sampleUsers.reduce((sum: number, user: any) => sum + user.TotalTasks, 0),
              BranchOverdueTasks: sampleUsers.reduce((sum: number, user: any) => sum + user.OverdueTasks, 0)
            }
          }
        })
      )
      ...
      console.log(`✅ Loaded ${branchesWithUsers.length} real branches with users from TMS`)

    } catch (err) {
      console.error('❌ Error fetching branches with users:', err)
      setError(`Failed to load TMS data: ${err instanceof Error ? err.message : 'Unknown error'}`)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Real-time data from your TMS - {branches.length} branches, {branches.reduce((sum, b) => sum + b.UserCount, 0)} users
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg">
          <div className="text-center">
            <div className="text-lg font-bold text-primary">{branches.length}</div>
          ...
          <div className="text-center">
            <div className="text-lg font-bold text-blue-600">
              {Math.round(branches.reduce((sum, b) => sum + b.BranchCompletionRate, 0) / Math.max(branches.length, 1))}%
            </div>
        ...
        <div className="space-y-3">
          {branches.map((branch) => {
            ...
            return (
              <Collapsible key={branch.BranchId} open={isExpanded} onOpenChange={() => toggleBranchExpansion(branch.BranchId)}>
                <Card className={cn(
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className={cn(
                            ...
                            <div className="font-bold text-sm">{branch.BranchHealthScore}%</div>
                            <div className="text-xs">Health Score</div>
                          </div>
                          
                          {/* Branch Performance */}
                          <div className="text-right">
                            <div className="font-bold text-sm">{branch.BranchCompletionRate}%</div>
                  ...
                  <CollapsibleContent>
                    <CardContent className="pt-0">
                      ...
                      <div className="grid grid-cols-4 gap-4 p-3 bg-muted/20 rounded-lg mb-4">
                        <div className="text-center">
                          <div className="font-bold text-green-600">{branch.TotalBranchTasks}</div>
                          <div className="text-xs text-muted-foreground">Total Tasks</div>
                        </div>
                        <div className="text-center">
                          <div className="font-bold text-blue-600">
                            {branch.Users.reduce((sum, u) => sum + u.CompletedTasks, 0)}
                          </div>
                          <div className="text-xs text-muted-foreground">Completed</div>
                        </div>
                        <div className="text-center">
                          <div className="font-bold text-red-600">{branch.BranchOverdueTasks}</div>
                          <div className="text-xs text-muted-foreground">Overdue</div>
                        </div>
                        <div className="text-center">
                          <div className="font-bold text-yellow-600">
                            {branch.Users.filter(u => u.CompletionRate >= 90).length}
                          </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {branch.Users.map(user => {
                          const performance = getPerformanceBadge(user.CompletionRate)
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                {/* Performance Badge */}
                                <Badge variant={performance.variant} className="text-xs min-w-[80px] text-center">
                                  {performance.label}
                                </Badge>
                                ...
                                {user.OverdueTasks > 0 && (
                                  <div className="flex items-center gap-1 text-red-600">
                                    ...
                                    <span className="text-xs">{user.OverdueTasks}</span>
                                  </div>
                                )}
                              </div>
                            </div>
