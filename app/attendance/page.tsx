...
const USER_API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL + "/users";
...
export default function AttendancePage() {
  ...
  const fetchAttendanceHistory = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // --- FIX START ---
        // Ensure 'records' is always an array, even if recordData is null
        let records = [];
        if (Array.isArray(recordData)) {
          records = recordData;
        } else if (recordData) {
          // Checks if recordData is truthy (not null/undefined)
          records = [recordData];
        }
        // --- FIX END ---
        ...
        setHistory(records); // This is now safe
      } else {
        setHistory([]);
      }
    } catch (error) {
  ...
  // Add Break In/Out blocked states using useMemo
  const isBreakInBlocked = useMemo(() => {
    return (
      isProcessing ||
      loading ||
      isButtonDisabled || // Location check
      !record?.CheckInAt || // Must be checked in first
      !!record?.BreakInAt || // Already on break
  ...
  const isBreakOutBlocked = useMemo(() => {
    return (
      isProcessing ||
      loading ||
      !record?.BreakInAt || // Must be on break first
      !!record?.BreakOutAt || // Already ended break
