...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store additional data for sidebar filtering
      localStorage.setItem("isAuthenticated", "true")
      localStorage.setItem("userRole", data.user.role)
      localStorage.setItem("userName", data.user.name)
      localStorage.setItem("userEmail", data.user.email)

      console.log("About to redirect for role:", data.user.role);
      setIsLoading(false)
      ...
      switch (data.user.role) {
        ...
        case "staff":
          ...
          break
        default:
          // If no valid role found, stay on login instead of defaulting to management
      ...
      <div className="flex items-center justify-between p-4 md:p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            <div className="text-gray-900 font-semibold text-lg">Marina Pharmacy</div>
            <div className="text-gray-500 text-sm">Task Management System</div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">
              ...
              <span className="text-emerald-600 font-semibold cursor-pointer hover:text-emerald-700">Get started</span>
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 rounded-xl p-5 mb-8 shadow-sm">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 mb-1">Demo Credentials</p>
                <p className="text-sm text-gray-700">
                  Use <span className="font-semibold text-gray-900">demo@marinapharma.com</span> with password{" "}
                  <span className="font-semibold text-gray-900">MyStrongPass!</span>
                </p>
              </div>
            </div>
          </div>
